...
module.exports.run = async (client, message, args) => {
 ...
 let lozÜye = message.mentions.members.first() || message.guild.members.get(args[0])
  ...
  let lozRol = message.guild.roles.find(`name`, "Susturulmuş");
  ...
  let lozZaman = args[1];
  if(!lozZaman) return message.channel.send("Lütfen doğru bir zaman dilimi giriniz. Örneğin: ***e!sustur @kişi 1s/m/h/d sebep**");
  let sebep = args[2]
  if(!sebep) return message.channel.send("Lütfen bir sebep giriniz. Örneğin: ***e!sustur @kişi 1s/m/h/d sebep**");

  await(lozÜye.addRole(lozRol.id));
  const kod = "```fix";
  const kod2 = "```";
   let embed = new Discord.RichEmbed()
              .setTitle("Kullanıcı Susturma Cezası Aldı")
                .setDescription(`
...
${lozZaman} ${kod2}`)
  ...
  let onay = message.guild.channels.find(`name`, "⛔ceza-log")
  message.guild.channels.get(onay.id).send(embed)
  
  setTimeout(function(){
    lozÜye.removeRole(lozRol.id);
    let sembed =  new Discord.RichEmbed()
              .setTitle("Kullanıcı Susturma Cezası Kalktı")
                .setDescription(`
